...
export function MissionBoard() {
  const { mercenaries, activeMission, setActiveMission, applyMissionResult, guild, items } =
    useGameStore();
  ...
  function handleResolve() {
    ...
    applyMissionResult({ ...result, materialsEarned: matDrops } as typeof result);
  }
            ...
            <div>
              ...
              {activeMission.consumablesAssigned && activeMission.consumablesAssigned.length > 0 && (
                <div className="text-xs text-stone-400 mt-1">
                  Consumables: {activeMission.consumablesAssigned.map((id) => (items[id] ?? ITEMS_MAP[id])?.name ?? id).join(', ')}
                </div>
              )}
                ...
                <>
                  ...
                  <div className="flex gap-3">
                    <button
                      ...
                      className="flex-1 py-2 rounded bg-amber-700 hover:bg-amber-600 disabled:opacity-40 disabled:cursor-not-allowed text-white font-medium text-sm transition-colors"
                    >
                      ⚔️ Send Party ({selectedMercIds.length} mercs{selectedConsumables.length > 0 ? `, ${selectedConsumables.length} consumable${selectedConsumables.length !== 1 ? 's' : ''}` : ''})
                    </button>
                    <button
                      onClick={() => setAssignStep('mercs')}
                      className="px-4 py-2 rounded bg-stone-700 hover:bg-stone-600 text-stone-200 text-sm transition-colors"
